...
def hacer_informe(lista_cajones, dict_precios):
    ...
    for line in lista_cajones:
        tupla = (line['nombre'], int(line['cajones']), float(line['precio']), round((float(dict_precios[line['nombre']]) - float(line['precio'])), 2))
        # print(f'Tupla: {tupla}')
...
for nombre, cajones, precio, cambio in lista_informe:
